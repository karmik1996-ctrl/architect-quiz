@echo off
echo ========================================
echo Push Firebase Modules to GitHub (FIX)
echo ========================================
echo.
cd /d D:\Programs\CursorTest\netlify-deploy
echo [1/5] Checking Firebase module files...
...
echo.
echo [3/5] Adding Firebase module files (force)...
...
echo Done!
echo.
echo [4/5] Checking status again...
git status js/firebase.config.js js/firebase.auth.js js/auth.ui.js js/quiz.access.js
echo.
...
if %errorlevel% neq 0 (
    echo.
    echo WARNING: Commit may have failed. Trying to push anyway...
    echo.
)
echo.
